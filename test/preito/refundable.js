...
export default function (Token, Crowdsale, wallets) {
  ...
  it('should withdraw, but send funds to dev and special wallets just once', async function () {
    ...
    const post = web3.eth.getBalance(this.wallet);
    post.minus(pre).plus(ether(22.5)).should.be.bignumber.equal(investment);
    await crowdsale.sendTransaction({value: ether(30), from: wallets[3]});
    await crowdsale.withdraw({from: owner}).should.be.fulfilled;
    const post1 = web3.eth.getBalance(this.wallet);
    post1.minus(post).should.be.bignumber.equal(ether(30));
  });
}
